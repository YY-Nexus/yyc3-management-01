    ...
    >
      <div className="relative">
        <div className="logo-pulse overflow-hidden rounded-lg">
          <Image
            src={darkMode ? "/images/yanyu-cloud-logo-dark.png" : "/images/yanyu-cloud-logo.png"}
            alt="言语云"
            width={width}
            height={height}
      ...
      {showText && (
        <div className="mt-4 text-center">
          <p className={cn("font-medium", textSize, darkMode ? "text-slate-800" : "text-white")}>{message}</p>
      ...
      {showSpinner && (
        <Loader2 className={cn("animate-spin mt-4", darkMode ? "text-slate-800" : "text-white")} size={spinnerSize} />
      )}
    </div>
  )
}
